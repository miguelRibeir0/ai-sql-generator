...
interface RoomsStockCardProps {
	roomsQuery: Products[];
}

export function RoomsStockCard({ roomsQuery }: RoomsStockCardProps) {
	return (
		<Card className="w-[90%] md:w-[70%]">
			<CardHeader>
				...
				<CardDescription>
					This is where we keep track of all products available and needed for
					the rooms.
				</CardDescription>
			</CardHeader>
			<CardContent>
				<Table>
					...
					<TableBody>
						{roomsQuery.map((item: Products, index: number) => (
							<TableRow key={index}>
